...
public final class RefactoringController implements IController{
	...
	private Runnable applyRefactoring() {
		return new Runnable() {				
			...
			public void run() {
				// begin custom code
				org.eclipse.emf.ecore.EEnumLiteral selectedEEnumLiteral = 
					(org.eclipse.emf.ecore.EEnumLiteral) dataManagement.
							getInPortByName(dataManagement.SELECTEDEOBJECT).getValue();
				String newName = 
					(String) dataManagement.getInPortByName("newName").getValue();
				selectedEEnumLiteral.setName(newName);
				// end custom code
			}
		};
	}
	...
	public final class InternalRefactoringProcessor extends 
		...
		public RefactoringStatus checkFinalConditions(){
				...
				if (selectedEEnumLiteral.getName().equals(newName)) {
					result.addFatalError("The new name is equal to the old name!");
				}
				// final check 2: the new name must be a valid EEnumLiteral name
				if (! RefactoringHelper.isValidEEnumLiteralName(newName)) {
					String message = 
							RefactoringHelper.getReasonForInvalidEEnumLiteralName(newName);
					result.addFatalError(message);
				}
				// final check 3: the containing EEnum must not contain an EEnumLiteral
				// with the specified name
				if (RefactoringHelper.
						containsEEnumLiteral(selectedEEnumLiteral.getEEnum(), newName)) {
					result.addFatalError("There is already an EEnumLiteral in the " +
											"containing EEnum named '" + newName + "'!");
				}
				// end custom code
				return result;
		}
