...
public final class RefactoringController implements IController{
	...
	public final class InternalRefactoringProcessor extends 
		...
		public RefactoringStatus checkFinalConditions(){
				...
				if (selectedEClass.getName().equals(newName)) {
					result.addFatalError("The new name is equal to the old name!");
				}
				// final check 2: the new name must be a valid EClass name
				if (! RefactoringHelper.isValidEClassName(newName)) {
					String message = 
							RefactoringHelper.getReasonForInvalidEClassName(newName);
					result.addFatalError(message);
				}
				// final check 3: the containing EPackage must not contain an EClassifier
				// with the specified name
				if (RefactoringHelper.
						containsEClassifier(selectedEClass.getEPackage(), newName)) {
