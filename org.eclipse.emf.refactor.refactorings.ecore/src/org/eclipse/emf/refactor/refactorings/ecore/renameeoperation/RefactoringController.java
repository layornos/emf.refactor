...
public final class RefactoringController implements IController{
	...
	private Runnable applyRefactoring() {
		return new Runnable() {				
			...
			public void run() {
				// begin custom code
				org.eclipse.emf.ecore.EOperation selectedEOperation = 
					(org.eclipse.emf.ecore.EOperation) dataManagement.
							getInPortByName(dataManagement.SELECTEDEOBJECT).getValue();
				String newName = 
					(String) dataManagement.getInPortByName("newName").getValue();
				selectedEOperation.setName(newName);
				// end custom code
			}
		};
	}
	...
	public final class InternalRefactoringProcessor extends 
		...
		public RefactoringStatus checkFinalConditions(){
				...
				if (selectedEOperation.getName().equals(newName)) {
					result.addFatalError("The new name is equal to the old name!");
				}
				// final check 2: the new name must be a valid EOperation name
				if (! RefactoringHelper.isValidEOperationName(newName)) {
					String message = 
							RefactoringHelper.getReasonForInvalidEOperationName(newName);
					result.addFatalError(message);
				}
				// final check 3: the containing EClass must not contain an EOperation
				// with the specified name and having the same signature 
				// (also no inherited EOperations)
				if (RefactoringHelper.containsEOperation
					(selectedEOperation.getEContainingClass(), selectedEOperation, newName)) {
