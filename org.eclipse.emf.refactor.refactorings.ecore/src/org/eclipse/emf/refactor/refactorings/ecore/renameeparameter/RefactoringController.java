...
public final class RefactoringController implements IController{
	...
	private Runnable applyRefactoring() {
		return new Runnable() {				
			...
			public void run() {
				// begin custom code
				org.eclipse.emf.ecore.EParameter selectedEParameter = 
					(org.eclipse.emf.ecore.EParameter) dataManagement.
							getInPortByName(dataManagement.SELECTEDEOBJECT).getValue();
				String newName = 
					(String) dataManagement.getInPortByName("newName").getValue();
				selectedEParameter.setName(newName);
				// end custom code
			}
		};
	}
	...
	public final class InternalRefactoringProcessor extends 
		...
		public RefactoringStatus checkFinalConditions(){
				...
				if (selectedEParameter.getName().equals(newName)) {
					result.addFatalError("The new name is equal to the old name!");
				}
				// final check 2: the new name must be a valid EParameter name
				if (! RefactoringHelper.isValidEParameterName(newName)) {
					String message = 
							RefactoringHelper.getReasonForInvalidEParameterName(newName);
					result.addFatalError(message);
				}
				// final check 3: the containing EOperation must not contain an EParameter
				// with the specified name 
				if (RefactoringHelper.containsEParameter
												(selectedEParameter.getEOperation(), newName)) {
					result.addFatalError("There is already an EParameter in the " +
											"containing EOperation named '" + newName + "'!");
				}
				// end custom code
				return result;
		}
