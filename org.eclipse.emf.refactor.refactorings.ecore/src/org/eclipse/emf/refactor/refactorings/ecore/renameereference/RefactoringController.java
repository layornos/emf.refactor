...
public final class RefactoringController implements IController{
	...
	private Runnable applyRefactoring() {
		return new Runnable() {				
			...
			public void run() {
				// begin custom code
				org.eclipse.emf.ecore.EReference selectedEReference = 
					(org.eclipse.emf.ecore.EReference) dataManagement.
							getInPortByName(dataManagement.SELECTEDEOBJECT).getValue();
				String newName = 
					(String) dataManagement.getInPortByName("newName").getValue();
				selectedEReference.setName(newName);
				// end custom code
			}
		};
	}
	...
	public final class InternalRefactoringProcessor extends 
		...
		public RefactoringStatus checkFinalConditions(){
				...
				if (selectedEReference.getName().equals(newName)) {
					result.addFatalError("The new name is equal to the old name!");
				}
				// final check 2: the new name must be a valid EReference name
				if (! RefactoringHelper.isValidEReferenceName(newName)) {
					String message = 
							RefactoringHelper.getReasonForInvalidEReferenceName(newName);
				...
				if (RefactoringHelper.containsEStructuralFeature
												(selectedEReference.getEContainingClass(), newName)) {
